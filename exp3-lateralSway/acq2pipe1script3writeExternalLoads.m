...
for iFiles = 1:nFiles
    ...
    LP = markerData(:,:,markerLabels == 'LP');
    RP = markerData(:,:,markerLabels == 'RP');
...
%     end
    ...
    tempZaxis = [0;0;1];
    tempZaxis = repmat(tempZaxis,1,length(d));
...
%     end
    ...
    % -----------------------------------------------------------
    % Create virtual marker for force origin
    % -----------------------------------------------------------
    vmRight = [RP(1,:); RP(2,:)+40; RP(3,:)];
    vmLeft = [LP(1,:); LP(2,:)-40; LP(3,:)];
    
    s1 = trialName{:};
    s2 = {'subject03_natural','subject04_trainer','subject06_trainer',...
            'subject07_limited','subject12_limited','subject12_natural',...
            'subject12_trainer'};
        
    if any(strcmp(s1,s2))
        clearvars vmRight vmLeft
        dispCalcToToeRight = rtoe - rcal;
        dispCalcToToeLeft = ltoe - lcal;
        % Set virtual marker pos as toe minus 1/3 of the disp. to calc in
        % global ref. frame
        vmRight = rtoe - dispCalcToToeRight * (1/3);
        vmLeft = ltoe - dispCalcToToeLeft * (1/3);
    ...
    for i = 1:length(rTanInt)
        A = reshape(rotationMatrix3d(i,:),3,3); %3x3
        B = vmRight(:,i); %3x1
        C = A*B; %3x1
        vmRightBikeXyz(1:3,i) = C;
        A = reshape(rotationMatrix3d(i,:),3,3); %3x3
        B = vmLeft(:,i); %3x1
        C = A*B; %3x1
        vmLeftBikeXyz(1:3,i) = C;
    end

    % Calculate crank angle using z-unit & virtual marker
    % Set inputs for angle3Points.
    p1 = [vmLeftBikeXyz(1,:)', vmLeftBikeXyz(3,:)' + ...
    ...
    % -----------------------------------------------------------
    % Calculate bicycle sway angle for later analysis
    % -----------------------------------------------------------
    % Note: output given in relation to global Y-Axis (QTM) in radians
    swayAngle = angle3Points(B2(1:2,:)',B3(1:2,:)',B3(1:2,:)'+[0;100]');
    swayAngle(swayAngle>1) = swayAngle(swayAngle>1)-(2*pi);
    swayAngleDelta = swayAngle-mean(swayAngle);
    swayAngleDeg = swayAngleDelta*(180/pi);
    ...
    [forceXyzBicycleRight,forceXyzBicycleLeft] = deal(zeros(2,length(rTanSmooth)));
    % Multiply crank force by 2D rotation matrix
    for i = 1:length(rTanInt)
        A = reshape(rotationMatrix2dRight(i,:),2,2);
        B = forceCrankRight(i,:);
        C = A*B';
        forceXyzBicycleRight(1:2,i) = C;
        A = reshape(rotationMatrix2dLeft(i,:),2,2);
        B = forceCrankLeft(i,:);
        C = A*B';
        forceXyzBicycleLeft(1:2,i) = C;
    end
    ...
    for i = 1:length(rTanInt)
        A = reshape(rotationMatrix3d(i,:),3,3);
        B = forceXyzBicycleRight(:,i);
        C = A*B;
        forceXyzGlobalRight(1:3,i) = C;
        A = reshape(rotationMatrix3d(i,:),3,3);
        B = forceXyzBicycleLeft(:,i);
        C = A*B;
        forceXyzGlobalLeft(1:3,i) = C;
    end
    ...
    % OpenSim(x) = QTM (x)
    pointXyzGlobalRight(1,:) = vmRight(1,:);
    pointXyzGlobalLeft(1,:) = vmLeft(1,:);
    % OpenSim(y) = QTM(z)
    pointXyzGlobalRight(2,:) = vmRight(3,:);
    pointXyzGlobalLeft(2,:) = vmLeft(3,:);
    % OpenSim(z) = QTM(-y)
    pointXyzGlobalRight(3,:) = -vmRight(2,:);
    pointXyzGlobalLeft(3,:) = -vmLeft(2,:);
